...
async function createSnippetCommand() {
  ...
  vscode.window.showInformationMessage(`Snippet "${title}" created \u{1F389}`);
}

// src/webviews/SnippetPanel.ts
...
var SnippetPanel = class {
  constructor(context) {
    this.context = context;
  }
  static viewType = "snippetshare.snippetPanel";
  _view;
  resolveWebviewView(webviewView, _context, _token) {
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      if (message.command === "loginSuccess" || message.command === "signupSuccess") {
  ...
  getHtmlForWebview(webview) {
    ...
    return (
      ...
      <head>
        <meta charset="UTF-8">
        <meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline' ${webview.cspSource}; script-src 'nonce-${nonce}'; connect-src http://localhost:8000;">
        ...
        <script nonce="${nonce}">
          ...
          document.getElementById('loginForm').addEventListener('submit', async (e) => {
            ...
            try {
              ...
              if (result.token) {
                vscode.postMessage({ command: 'loginSuccess', token: result.token });
              } else {
                document.getElementById('error').innerText = result.error || "Unknown error";
              }
            } catch (err) {
          ...
          document.getElementById('signupForm').addEventListener('submit', async (e) => {
            ...
            try {
              const result = await callBackend("/signup", { email, password });
              if (result.token) {
                vscode.postMessage({ command: 'signupSuccess', token: result.token });
              } else {
                document.getElementById('error').innerText = result.error || "Unknown error";
              }
            } catch (err) {
...
async function fetchWorkspaces() {
  ...
  const response = await fetch2("http://localhost:8000/api/workspaces", {
    method: "GET",
    headers: {
...
function setFirebaseToken(token) {
  firebaseToken = token;
  vscode3.window.showInformationMessage("\u{1F511} Firebase token saved!");
}
function getFirebaseToken() {
  return firebaseToken;
}
function activate(context) {
  console.log("\u{1F389} SnippetShare extension is now active!");
    ...
    vscode3.commands.registerCommand(
      ...
      (code) => {
        ...
        if (editor) {
          ...
          vscode3.window.showErrorMessage("No active text editor found.");
        }
      }
    )
  );
  ...
  context.subscriptions.push(
    vscode3.window.registerWebviewViewProvider(SnippetPanel.viewType, panel)
  );
  context.subscriptions.push(
    vscode3.commands.registerCommand(
      "snippetshare.handleAuth",
      async (token) => {
        ...
        try {
          ...
          vscode3.window.showInformationMessage(
            `Fetched ${workspaces.length} workspaces!`
          );
        } catch (err) {
